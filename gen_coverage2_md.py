...
CHANGES_SINCE = {
    ...
    6: "Since iteration 5: **`lf_diff_comment_names_concurrency_bug`** landed "
       "(mechanism D in `Progress.md`) — an added *code comment* naming a race, "
       "use-after-free, double-free, deadlock or TOCTOU. Patch comments are "
       "franker than commit messages and nothing else read them. Gold votes "
       "**36 → 37**, majority vote **33 → 34**, and the LabelModel moved for "
       "the first time in four iterations: **11 → 12**. Read the vote-count "
       "table below before drawing the obvious conclusion, though: the row this "
       "LF was built for (`c67b59f891`) still sits at `prob_security` = 0.24 "
       "and is *not* labeled. The extra label is `9bb2878319`, which this LF "
       "gave a **second** positive vote, pushing it to 0.536.",
    5: "Since iteration 4: **`lf_patch_cites_fix_commit`** landed (mechanism C "
       "in `Progress.md`), fed by the new `fetch_patch_citations.py`. Release "
       "commits generated by `release-please`/`changesets` link the fixes they "
       "ship (`* **delta:** omit dangerous URL by default ([2817147](…))`), so "
       "the changelog names a commit the positive LFs already voted on. "
       "Measured at 49 non-merge firings for **2 gold hits**; gold votes "
       "**34 → 36**. Half of mechanism C was **rejected on measurement**: a "
       "sibling detector for CVE/GHSA ids in changelog text scores 18 firings "
       "and 0 gold without merges, or 1 gold for 344 merge firings with them, "
       "because craftcms CHANGELOG lines propagate through every branch merge. "
...
full = features.add_all_features(full)
...
stand_cols = [col[lf.name] for lf in DIFF_STANDALONE_LFS]
# Every SECURITY-polarity column. Anything that counts "does this row have a
# positive vote" must use this, or adding an LF outside the existing groups
# silently fails to count - which is exactly what happened when the standalone
# comment LF was added.
all_pos_cols = pos_msg_cols + mit_cols + conj_cols + stand_cols
...
def vote_count_table(cols):
    """How many co-firing positives does a row need before the model labels it?"""
    sub = L[:, cols]
    lm = LabelModel(cardinality=2, verbose=False)
    buf = io.StringIO()
    with redirect_stdout(buf), redirect_stderr(buf):
        lm.fit(L_train=sub, n_epochs=args.epochs, lr=0.01, seed=args.seed,
               class_balance=[1 - args.class_balance, args.class_balance])
        probs = lm.predict_proba(L=sub)[:, SECURITY]
    nvotes = (L[:, all_pos_cols] == SECURITY).sum(axis=1)
    conv = probs >= args.threshold
    rows = []
    for k in (0, 1, 2, 3):
        m = (nvotes == k) if k < 3 else (nvotes >= 3)
        rows.append({
            "votes": str(k) if k < 3 else "3+",
            "rows": int(m.sum()), "converted": int((m & conv).sum()),
            "gold_rows": int((m & (gold == 1)).sum()),
            "gold_converted": int((m & conv & (gold == 1)).sum()),
        })
    return rows
...
ablations = [
    ...
    fit(msg_cols + mit_cols + conj_cols + stand_cols,
        "message + mitigation + conjunction + standalone"),
    fit(msg_cols + diff_cols,
        f"all {len(ALL_LFS_WITH_DIFF)} (adds the 6 surface vetoes)"),
    fit(diff_cols, "diff LFs only"),
]

votes_table = vote_count_table(msg_cols + diff_cols)
...
w(f"| Gold fixes with any positive vote | {_msg_pos_gold} of 49 | "
  f"{int((L[np.ix_(gold_idx, all_pos_cols)] == SECURITY).any(axis=1).sum())} "
  "of 49 |")
...
n_diffpos_gold = int((L[np.ix_(gold_idx, mit_cols + conj_cols + stand_cols)]
                      == SECURITY).any(axis=1).sum())
...
for lf in DIFF_LFS:
    j = col[lf.name]
    pol = NOT_SEC if role.get(lf.name) == "surface" else SECURITY
    ...
    w(f"| `{lf.name}` | {'NOT_SEC' if pol == NOT_SEC else 'SECURITY'} | "
      f"{role.get(lf.name, 'standalone')} | {fired:,} | "
      f"{pct(k / len(control_idx))} ({k}) | "
      f"{pct(lo)}–{pct(hi)} | {gh} |")
w("")
...
w("")
_lone = 3
w(f"**Why majority vote no longer tracks the vote count.** {_lone} gold rows "
  "now carry exactly one positive vote against one or two negatives — "
  "`c05684d9e7` (1 vs `lf_version_bump` + `lf_feature_commit`), `b636a220d8` "
  "and `b6f613b1e3` (1 vs `lf_bot_author`) — so majority vote reads them as "
  "not-security or ties and abstains. They are all release commits, which is "
  "where mechanisms B and C do their work: the derived LFs and the "
  "release-shaped negatives are now fighting over the same rows. Withholding "
  "those three negatives when a derived release feature fires would move "
  "majority vote from 33 to 36; it would also weaken three LFs whose learned "
  "accuracy is 1.00, so it is a deliberate trade rather than a free win.")
w("")
...
w("### How many positive votes does a label cost?")
w("")
w("Same fitted model as the last ablation row, bucketed by how many "
  "SECURITY-polarity LFs co-fire on a row:")
w("")
w("| Co-firing positives | Rows | Labeled `security` | Gold rows | Gold labeled |")
w("| --- | --- | --- | --- | --- |")
for _v in votes_table:
    w(f"| {_v['votes']} | {_v['rows']:,} | {_v['converted']:,} | "
      f"{_v['gold_rows']} | {_v['gold_converted']} |")
w("")
_one = next(v for v in votes_table if v["votes"] == "1")
w(f"**A single positive vote is worth nothing.** {_one['rows']:,} rows carry "
  f"exactly one and {_one['converted']} are labeled `security` — not a rounding "
  "error, an exact zero, corpus-wide. Rows with two or more convert at roughly "
  "three in four. That is the whole conversion story: every mechanism shipped "
  "so far adds *lone* votes to rows that had none, which is why the vote count "
  "climbs and the label count barely moves. Two ways out, and only two: make "
  "positives co-fire on the same row, or change the model configuration so one "
  "vote can clear the threshold.")
w("")
