...
Applies every LF in ALL_LFS_WITH_DIFF to data/interim/diff_sample.parquet -
...
p.add_argument("--iteration", type=int, default=3,
               help="iteration number; also picks the default --out filename "
                    "and the 'what changed' preamble")
...
args = p.parse_args()
if args.out is None:
    args.out = ROOT / f"Coverage_Iteration{args.iteration}.md"

# Keyed by iteration so re-running the same command reproduces the same
# document. Each entry states what changed since the previous report and the
# measured effect - add a new key when you land a mechanism.
CHANGES_SINCE = {
    2: "Iteration 1 measured the 20 shipped message LFs on the full corpus and "
    ...
    3: "Since iteration 2: **`lf_security_note_path`** landed (mechanism A in "
       ...
       "546 firings (0.76%) for **7 gold hits**, all of them cpython "
       "`gh-151987` rows, which had been the largest single block of missed "
       "gold. Gold fixes with a positive vote went **22 → 29**; majority vote "
       "**22 → 29**; LabelModel conversion **did not move** (11 either way), "
       "because the new LF's learned accuracy is 0.274 like every other "
       "positive, so its single vote yields `prob_security` ≈ 0.27 and loses "
...
ablations = [
    fit(msg_cols, f"message/metadata LFs only ({len(ALL_LFS)})"),
    fit(msg_cols + mit_cols, "message + 6 mitigation LFs"),
    fit(msg_cols + mit_cols + conj_cols, "message + mitigation + conjunction"),
    fit(msg_cols + diff_cols,
        f"all {len(ALL_LFS_WITH_DIFF)} (adds the 6 surface vetoes)"),
...
w(f"# LF coverage — iteration {args.iteration} (diff-aware LFs)")
w("")
if args.iteration in CHANGES_SINCE:
    w(CHANGES_SINCE[args.iteration])
    w("")
w(f"Measured over **{len(ALL_LFS_WITH_DIFF)} LFs** — {len(ALL_LFS)} "
  f"message/metadata + {len(DIFF_LFS)} diff — on the "
  f"{len(df):,}-row stratified diff sample. Generated by "
  f"`gen_coverage2_md.py --iteration {args.iteration}`.")
w("")
...
w("## Headline")
w("")
w("| Metric | message/metadata only | + diff LFs |")
w("| --- | --- | --- |")
base, best = ablations[0], ablations[3]
w(f"| LFs | {len(ALL_LFS)} | {len(ALL_LFS_WITH_DIFF)} |")
_msg_pos_gold = int((L[np.ix_(gold_idx, pos_msg_cols)] == SECURITY).any(axis=1).sum())
w(f"| Gold fixes with any positive vote | {_msg_pos_gold} of 49 | "
...
w("")
w(f"*(Both columns are computed on these same {len(df):,} sampled rows, so the "
  "comparison is apples-to-apples. Neither is the full-corpus fit — see "
  "`gen_coverage_md.py` for that.)*")
w("")
...
w(f"## Snorkel `LFAnalysis`, all {len(ALL_LFS_WITH_DIFF)} LFs")
w("")
...
w(f"## What to change in iteration {args.iteration + 1}")
w("")
...
w("3. **The surface vetoes barely matter.** Adding them changed neither gold "
  ...
  "tuning next.")
w(f"4. **The bottleneck is still the LabelModel, not the LFs.** Majority vote "
  f"reaches {best['mv_gold']} of 49; the fitted model converts "
  f"{best['gold']}. Iteration 1 diagnosed why "
